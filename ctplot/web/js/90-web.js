...
(function ($) {
  ...
  function sourcesBox() {
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        $("<option>")
          .val("")
          .text("(bitte Experiment auswählen)")
          .appendTo(experimentbox);
        datasetbox = $("<select>").attr("name", "s*");
        $("<option>")
          .val("")
          .text("(bitte Datensatz auswählen)")
          .appendTo(datasetbox);
        ...
        $.each(data, function (id, info) {
          ...
          if (
            ...
            var ex = $("<option>")
              ...
              .val(experiment)
              .appendTo(experimentbox);
            if (experiment.startsWith("x")) ex.addClass("expert");
          }

          if (opt.text().startsWith("x")) opt.addClass("expert");
        });
      },
  ...
  function renumberPlots() {
    ...
    if (ch.length >= 4) $("#addplot").hide();
    else $("#addplot").show();

    // hide/show twin axis dropdownbox
    if (ch.length < 2)
      $('.plot :input[name^="tw"]').parent("label").addClass("hidden");
    else $('.plot :input[name^="tw"]').parent("label").removeClass("hidden");
  ...
  function updateHiddenFields() {
    ...
    $.each(["x", "y"], function (i, v) {
      ...
      if (
        $(':input[name^="tw"] option:selected[value="' + v + '"]').size() == 0
      ) {
    ...
    $(".plot").each(function () {
      ...
      if (experiment.match(/\s+/)) experiment = "";
      console.debug("experiment=" + experiment);
  ...
  function updateAxisVarsDropdowns(plot) {
    plot
      .find(':input[name^="x"],:input[name^="y"],:input[name^="z"]')
      .each(function () {
  ...
  function addHandlers(plot) {
    ...
    plot.find(':input[name$="color"]').each(function () {
      var input = $(this),
        parent = input.parent(),
        picker = $('<div class="colorselector"><div></div></div>').attr(
          "title",
          parent.data("help")
        );
  ...
  function getSettings() {
    ...
    $("form :input:enabled:not(:button):not(:reset):not(:submit)[name]").each(
      function () {
        var field = $(this),
  ...
  function addPlotToSaved(settings) {
    ...
    var plotImg = $(
        '<img src="' +
          ...
          '" title="' +
          settings.t +
          '" class="savedPlotImage">'
      ),
      delBtn = $('<i class="del-btn inverse"></i>').attr(
        "title",
        "Diagramm löschen"
      ),
      loadBtn = $('<i class="fa fa-repeat fa-2x icon-green"></i>').attr(
        "title",
        "Plot laden"
      ),
  ...
  function initSubmit() {
    ...
    $("form").submit(function (e) {
      ...
      result
        .empty()
        .append(
          '<p class="text-centered">Diagramm wird erstellt, bitte warten&hellip;<br /><br /><img src="img/bar90.gif"></p>'
        );
      $("#error").empty();
      ...
      xhr = $.ajax({
        ...
        success: function (data) {
          if (data.errors) {
            ...
            if (errors.diagrams) {
              $.each(errors.diagrams, function (dataset, dsErrors) {
                if (dsErrors.length < 1) return;

                var dataset = parseInt(dataset) + 1,
                  heading = $(
                    "<h3>Fehler in der " + dataset + ". Datenreihe:</h3>"
                  );
          ...
          $("<img>")
            .attr("src", img + "?t=" + new Date().getTime())
            .attr("alt", query)
            .attr("id", "plotImage")
            .appendTo(result);
        ...
        error: function (xhr, text, error) {
          ...
          errorbox.html(
            "<p>Es ist ein unbekannter Fehler aufgetreten. Bitte überprüfe deine Eingaben und versuche es erneut.</p>"
          );
